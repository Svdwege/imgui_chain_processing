...
template <typename T>
inline T ImCubicBezier(const T &p0, const T &p1, const T &p2, const T &p3,
                       float t) {
  const auto a = 1 - t;
  const auto b = a * a * a;
  const auto c = t * t * t;
...
template <typename T>
inline T ImCubicBezierDt(const T &p0, const T &p1, const T &p2, const T &p3,
                         float t) {
  const auto a = 1 - t;
  const auto b = a * a;
  const auto c = t * t;
...
template <typename T>
inline T ImCubicBezierSample(const T &p0, const T &p1, const T &p2, const T &p3,
                             float t) {
...
template <typename T>
inline T ImCubicBezierTangent(const T &p0, const T &p1, const T &p2,
                              const T &p3, float t) {
  const auto cp0_zero = ImLengthSqr(p1 - p0) < 1e-5f;
  const auto cp1_zero = ImLengthSqr(p3 - p2) < 1e-5f;
...
template <typename T>
inline float ImCubicBezierLength(const T &p0, const T &p1, const T &p2,
                                 const T &p3) {
  // Legendre-Gauss abscissae with n=24 (x_i values, defined at i=n as the roots
  // of the nth order Legendre polynomial Pn(x))
  static const float t_values[] = {-0.0640568928626056260850430826247450385909f,
  ...
  // to in the Bezier primer article)
  static const float c_values[] = {0.1279381953467521569740561652246953718517f,
  ...
  static_assert(sizeof(t_values) / sizeof(*t_values) ==
                    sizeof(c_values) / sizeof(*c_values),
                "");
...
template <typename T>
inline ImCubicBezierSplitResultT<T> ImCubicBezierSplit(const T &p0, const T &p1,
  ...
  return ImCubicBezierSplitResultT<T>{
      ImCubicBezierPointsT<T>{
          p0, z1 * p1 - s1 * p0, z2 * p2 - 2 * z1 * s1 * p1 + s2 * p0,
...
template <typename T>
inline ImCubicBezierSplitResultT<T>
ImCubicBezierSplit(const ImCubicBezierPointsT<T> &curve, float t) {
  return ImCubicBezierSplit(curve.P0, curve.P1, curve.P2, curve.P3, t);
}

inline ImRect ImCubicBezierBoundingRect(const ImVec2 &p0, const ImVec2 &p1,
...
#define IM_VEC2_INDEX(v, i) *(&v.x + i)
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (IM_VEC2_INDEX(delta_squared, i) >= 0) {
      ...
      if (t0 > 0 && t0 < 1) {
        auto p = ImCubicBezier(IM_VEC2_INDEX(p0, i), IM_VEC2_INDEX(p1, i),
                               IM_VEC2_INDEX(p2, i), IM_VEC2_INDEX(p3, i), t0);
      ...
      if (t1 > 0 && t1 < 1) {
        auto p = ImCubicBezier(IM_VEC2_INDEX(p0, i), IM_VEC2_INDEX(p1, i),
                               IM_VEC2_INDEX(p2, i), IM_VEC2_INDEX(p3, i), t1);
        IM_VEC2_INDEX(tl, i) = ImMin(IM_VEC2_INDEX(tl, i), p);
        IM_VEC2_INDEX(rb, i) = ImMax(IM_VEC2_INDEX(rb, i), p);
      }
    }
...
inline ImProjectResult
ImProjectOnCubicBezier(const ImVec2 &point, const ImVec2 &p0, const ImVec2 &p1,
                       const ImVec2 &p2, const ImVec2 &p3,
...
inline ImProjectResult ImProjectOnCubicBezier(const ImVec2 &p,
                                              const ImCubicBezierPoints &curve,
                                              const int subdivisions) {
  return ImProjectOnCubicBezier(p, curve.P0, curve.P1, curve.P2, curve.P3,
                                subdivisions);
}

inline ImCubicBezierIntersectResult
ImCubicBezierLineIntersect(const ImVec2 &p0, const ImVec2 &p1, const ImVec2 &p2,
                           const ImVec2 &p3, const ImVec2 &a0,
                           const ImVec2 &a1) {
  auto cubic_roots = [](float a, float b, float c, float d,
                        float *roots) -> int {
    int count = 0;
    ...
    {
      auto S =
          sign(R + ImSqrt(D)) * ImPow(ImFabs(R + ImSqrt(D)), (1.0f / 3.0f));
      auto T =
          sign(R - ImSqrt(D)) * ImPow(ImFabs(R - ImSqrt(D)), (1.0f / 3.0f));
...
*/
  ...
  float roots[3];
  auto rootCount =
      cubic_roots(a * c3.x + b * c3.y, a * c2.x + b * c2.y, a * c1.x + b * c1.y,
                  a * c0.x + b * c0.y + c, roots);
...
inline ImCubicBezierIntersectResult
ImCubicBezierLineIntersect(const ImCubicBezierPoints &curve,
                           const ImLine &line) {
  return ImCubicBezierLineIntersect(curve.P0, curve.P1, curve.P2, curve.P3,
                                    line.A, line.B);
}

inline void ImCubicBezierSubdivide(ImCubicBezierSubdivideCallback callback,
                                   void *user_pointer, const ImVec2 &p0,
                                   const ImVec2 &p1, const ImVec2 &p2,
                                   const ImVec2 &p3, float tess_tol,
                                   ImCubicBezierSubdivideFlags flags) {
  return ImCubicBezierSubdivide(callback, user_pointer,
                                ImCubicBezierPoints{p0, p1, p2, p3}, tess_tol,
                                flags);
}
...
inline void ImCubicBezierSubdivide(ImCubicBezierSubdivideCallback callback,
  ...
  struct Tesselator {
    ...
    void Subdivide(const ImCubicBezierPoints &curve, int level = 0) {
      ...
      float d2 =
          ((curve.P1.x - curve.P3.x) * dy - (curve.P1.y - curve.P3.y) * dx);
      float d3 =
          ((curve.P2.x - curve.P3.x) * dy - (curve.P2.y - curve.P3.y) * dx);
      d2 = (d2 >= 0) ? d2 : -d2;
      d3 = (d3 >= 0) ? d3 : -d3;
...
template <typename F>
inline void ImCubicBezierSubdivide(F &callback, const ImVec2 &p0,
                                   const ImVec2 &p1, const ImVec2 &p2,
                                   const ImVec2 &p3, float tess_tol,
                                   ImCubicBezierSubdivideFlags flags) {
  ...
  ImCubicBezierSubdivide(handler, &callback,
                         ImCubicBezierPoints{p0, p1, p2, p3}, tess_tol, flags);
}

template <typename F>
inline void
ImCubicBezierSubdivide(F &callback, const ImCubicBezierPoints &curve,
...
inline void ImCubicBezierFixedStep(ImCubicBezierFixedStepCallback callback,
                                   void *user_pointer, const ImVec2 &p0,
                                   const ImVec2 &p1, const ImVec2 &p2,
                                   const ImVec2 &p3, float step, bool overshoot,
  ...
  const auto point_count =
      static_cast<int>(total_length / step) + (overshoot ? 2 : 1);
  ...
  for (int point_index = 1; point_index < point_count; ++point_index) {
    ...
    while (true) {
      ...
      if (ImFabs(error) <= max_value_error ||
          ImFabs(t_start - t_end) <= max_t_error) {
        sample.T = t;
...
inline void ImCubicBezierFixedStep(ImCubicBezierFixedStepCallback callback,
                                   void *user_pointer,
                                   const ImCubicBezierPoints &curve, float step,
                                   bool overshoot, float max_value_error,
                                   float max_t_error) {
  ImCubicBezierFixedStep(callback, user_pointer, curve.P0, curve.P1, curve.P2,
                         curve.P3, step, overshoot, max_value_error,
                         max_t_error);
}

// F has signature void(const ImCubicBezierFixedStepSample& p)
template <typename F>
inline void ImCubicBezierFixedStep(F &callback, const ImVec2 &p0,
                                   const ImVec2 &p1, const ImVec2 &p2,
                                   const ImVec2 &p3, float step, bool overshoot,
                                   float max_value_error, float max_t_error) {
  ...
  ImCubicBezierFixedStep(handler, &callback, p0, p1, p2, p3, step, overshoot,
                         max_value_error, max_t_error);
}

template <typename F>
inline void ImCubicBezierFixedStep(F &callback,
                                   const ImCubicBezierPoints &curve, float step,
                                   bool overshoot, float max_value_error,
                                   float max_t_error) {
  auto handler = [](ImCubicBezierFixedStepSample &sample, void *user_pointer) {
  ...
  ImCubicBezierFixedStep(handler, &callback, curve.P0, curve.P1, curve.P2,
                         curve.P3, step, overshoot, max_value_error,
                         max_t_error);
}
