...
// #define ConnectionFilter_None       [](ImFlow::Pin* out, ImFlow::Pin* in){
// return true; } #define ConnectionFilter_SameType   [](ImFlow::Pin* out,
// ImFlow::Pin* in){ return out->getDataType() == in->getDataType(); } #define
// ConnectionFilter_Numbers    [](ImFlow::Pin* out, ImFlow::Pin* in){ return
// out->getDataType() == typeid(double) || out->getDataType() == typeid(float)
// || out->getDataType() == typeid(int); }
...
private:
   ...
   */
  template <typename L, typename B = BaseNode> struct NodeWrapper : public B {
    L mLambda;
...
public:
   ...
   */
  std::vector<std::string> &get_recursion_blacklist() {
...
public:
   ...
   */
  template <typename T>
  std::shared_ptr<InPin<T>> addIN(const std::string &name, T defReturn,
  ...
  addOUT(const std::string &name, std::shared_ptr<PinStyle> style = nullptr);
...
public:
   ...
   * @param style Style of the pin
   */
  explicit Pin(PinUID uid, std::string name, std::shared_ptr<PinStyle> style,
               PinType kind, BaseNode *parent, ImNodeFlow **inf)
      : m_uid(uid), m_name(std::move(name)), m_type(kind), m_parent(parent),
        m_inf(inf), m_style(std::move(style)) {
    if (!m_style)
...
class ConnectionFilter {
public:
  static std::function<bool(Pin *, Pin *)> None() {
    return [](Pin *out, Pin *in) { return true; };
  }
  static std::function<bool(Pin *, Pin *)> SameType() {
    return [](Pin *out, Pin *in) {
      return out->getDataType() == in->getDataType();
    };
  }
  static std::function<bool(Pin *, Pin *)> Numbers() {
    return [](Pin *out, Pin *in) {
      return out->getDataType() == typeid(double) ||
...
public:
  ...
  explicit InPin(PinUID uid, const std::string &name, T defReturn,
                 std::function<bool(Pin *, Pin *)> filter,
                 std::shared_ptr<PinStyle> style, BaseNode *parent,
                 ImNodeFlow **inf)
      : Pin(uid, name, style, PinType_Input, parent, inf),
        m_emptyVal(defReturn), m_filter(std::move(filter)) {}
   ...
   */
  [[nodiscard]] const std::function<bool(Pin *, Pin *)> &getFilter() const {
    return m_filter;
  }
   ...
   * @return String containing unique information identifying the data type
   */
  [[nodiscard]] const std::type_info &getDataType() const override {
    return typeid(T);
  };
...
public:
   ...
   * @param style Style of the pin
   */
  explicit OutPin(PinUID uid, const std::string &name,
                  std::shared_ptr<PinStyle> style, BaseNode *parent,
                  ImNodeFlow **inf)
