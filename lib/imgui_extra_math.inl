...
//------------------------------------------------------------------------------
inline float ImLength(float v) { return v; }

inline float ImLength(const ImVec2 &v) { return ImSqrt(ImLengthSqr(v)); }

inline float ImLengthSqr(float v) { return v * v; }

inline ImVec2 ImNormalized(const ImVec2 &v) { return v * ImInvLength(v, 0.0f); }
...
inline ImVec2 ImRect_ClosestPoint(const ImRect &rect, const ImRect &other) {
  ...
  if (other.Min.x >= rect.Max.x)
    ...
    result.x =
        (ImMax(rect.Min.x, other.Min.x) + ImMin(rect.Max.x, other.Max.x)) / 2;
  ...
  if (other.Min.y >= rect.Max.y)
    ...
    result.y =
        (ImMax(rect.Min.y, other.Min.y) + ImMin(rect.Max.y, other.Max.y)) / 2;
...
inline ImLine ImRect_ClosestLine(const ImRect &rect_a, const ImRect &rect_b) {
  ...
  auto distribute = [](float &a, float &b, float a0, float a1, float b0,
                       float b1) {
    if (a0 >= b1 || a1 <= b0)
      return;
  ...
  distribute(result.A.x, result.B.x, rect_a.Min.x, rect_a.Max.x, rect_b.Min.x,
             rect_b.Max.x);
  distribute(result.A.y, result.B.y, rect_a.Min.y, rect_a.Max.y, rect_b.Min.y,
             rect_b.Max.y);
